...
# -*- coding: utf-8 -*-
"""
This code was taken from Kaggle's kernel found at the link
https://www.kaggle.com/chattob/cervix-segmentation-gmm
and was essentially written bythe user chattob. It has been
modified very slightly to be compatible with the remaining code.#
The relevant function is crop_image.
"""
...
def crop_image(input_imagearray):
    """
    Requires as input an image array where the pixels take on values between
    0 and 255. Taken an image array and outputs an image array
    """
    img = input_imagearray.astype(np.uint8)
    ...
    labels = g.predict(Ra)
    # Add 1 to avoid labeling as 0 since regionprops ignores the 0-label.
    labels += 1
    ...
    # To draw the contour (no assignement to variable necessary):
    # cv2.drawContours(img, main_contour, -1, 255, 3)

    x, y, w, h = cv2.boundingRect(main_contour)

    # If we want to have the rectangle data
